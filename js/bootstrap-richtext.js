...
function richtextpreview(id, userOptions) {
	...
	var html = '<div class="tab-pane switch-icon">\r\n' + 
			...
			'</div>\r\n' + 
			'<div class="tab-pane' + von + '" id="' + oid + '-v" ><div id="' + oid + '-v-e" class="form-control" style="height: ' + __getHeight(id) + 'px;overflow-y: scroll;" title="' + ($(id).attr("title") || $(id).attr("placeholder") || "") + '"></div>\r\n' + 
			'</div>\r\n';
...
function richtextcode(id, userOptions) {
	...
	var html = '<div class="switch-icon">\r\n' + 
			...
			'</div>\r\n' + 
			'<div class="tab-pane mvc-group-v active" id="' + oid + '-v" ><div id="' + oid + '-v-e" target-obj="' + id + '" class="editable" style="height: ' + __getHeight(id) + 'px;" title="' + ($(id).attr("title") || $(id).attr("placeholder") || "") + '"></div>\r\n' + 
			'</div>\r\n';
	$(html).insertBefore('#' + oid + '-m');
	html = '<div class="message hide"></div>\r\n';
	$(html).insertAfter('#' + oid + '-m');
...
function __richtext(id, userOptions) {
	...
	$(id).wrap( "<div class='richtext-zone " + ($(id).attr("class").indexOf("col-") > -1 ? $(id).attr("class").replace(/(.*)(col\-[^ ]+)(.*)/gi, "$2") : "") + "'></div>");
	$('<div target-obj="' + id + '" class="editable" style="height: ' + __getHeight(id) + 'px;" title="' + ($(id).attr("title") || $(id).attr("placeholder") || "") + '"></div>').insertBefore(id);
	$(id).hide();
